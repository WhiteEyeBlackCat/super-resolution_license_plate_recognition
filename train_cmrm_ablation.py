...
MODEL_PATH = str(PROJECT_ROOT / "Qwen3-VL" / "Qwen3-VL-8B-Instruct")
UFPR_ROOT_DIR = PROJECT_ROOT / "UFPR-SR-plates" / "1920_1080"
OUTPUT_ROOT = PROJECT_ROOT / "model" / "outputs" / "cmrm_ablation_mvc_only"
...
EXPERIMENTS = [
    ...
    {
        "name": "03_lora_cmrm_reg_mvc",
        "title": "LoRA + CMRM (loss_reg + loss_mvc)",
        "use_cmrm": True,
        "train_lora": True,
        "train_cmrm": True,
        "lambda_reg": 1.0,
...
def summarize_history(history: List[dict], config: dict, dataset_summary: dict) -> dict:
    ...
    if best_val is not None:
        summary["best_val"] = {
            ...
            "mvc": float(best_val.get("mvc", 0.0)),
        }
    ...
    if last_train is not None:
        summary["last_train"] = {
            key: float(last_train.get(key, 0.0)) if key in {"total", "reg", "mvc"} else int(last_train[key])
            for key in ["step", "epoch", "total", "reg", "mvc"]
        }
    if last_val is not None:
        summary["last_val"] = {
            key: float(last_val.get(key, 0.0)) if key in {"total", "reg", "mvc"} else int(last_val[key])
            for key in ["step", "epoch", "total", "reg", "mvc"]
        }
...
def plot_history(history, save_dir, name, reg_weight=1.0, lambda_mvc=1.0):
    ...
    if train_records:
        ...
        train_mvc_w = [lambda_mvc * r.get("mvc", 0.0) for r in train_records]
        ...
        plt.plot(x, train_mvc_w, marker="o", label=f"train_mvc*x{lambda_mvc}")
    ...
    if val_records:
        ...
        val_mvc_w = [lambda_mvc * r.get("mvc", 0.0) for r in val_records]
        ...
        plt.plot(x, val_mvc_w, marker="o", label=f"val_mvc*x{lambda_mvc}")
    ...
    mvc_records = [r for r in history if r.get("split") in {"train", "val", "val_epoch_end"} and r.get("mvc") is not None]
    if mvc_records:
        xs = [r["step"] for r in mvc_records]
        mvcs = [r["mvc"] for r in mvc_records]
        plt.figure(figsize=(9, 5))
        plt.plot(xs, mvcs, marker="o", label="mvc_loss")
        plt.title(f"{name} Multi-View Consistency Loss")
        plt.xlabel("Step")
        plt.ylabel("Loss")
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.tight_layout()
        plt.savefig(save_dir / f"{name}_mvc_loss.png", dpi=200)
        plt.close()
...
def plot_combined_comparison(results: List[dict], save_dir: Path):
    ...
    best_mvc = [result["summary"].get("best_val", {}).get("mvc", math.nan) for result in results]

    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    metrics = [
        (best_val, "Best Validation Total", "Loss"),
        (best_mvc, "Best Validation MVC", "Loss"),
    ...
    for ax, (values, title, ylabel) in zip(axes.flat[: len(metrics)], metrics):
            ...
            writer.writerow(
                {
                    ...
                    "best_val_mvc": best_val.get("mvc", math.nan),
